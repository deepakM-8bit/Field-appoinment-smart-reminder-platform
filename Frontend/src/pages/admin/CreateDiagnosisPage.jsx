  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="relative mb-6 w-64">
        ...
        {/* Dropdown */}
        {showDropdown && (
          <div className="absolute z-10 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-950">
            {suggestions.length > 0 ? (
              suggestions.map((c) => (
                <button
                  key={c.id}
                  onClick={() => selectCustomer(c)}
                  className="flex w-full justify-between px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800/50"
                >
      ...
      <div className="mb-6">
        <h3 className="mb-2 text-sm font-medium text-slate-800 dark:text-slate-200">
          Customer Details{" "}
          {customerFound && (
            <span className="ml-2 rounded bg-green-100 px-2 py-0.5 text-xs text-green-700 dark:bg-green-500/10 dark:text-green-300">
              Existing customer
            </span>
          )}
        </h3>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <input
            className="rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400
                       focus:outline-none focus:ring-2 focus:ring-blue-600
                       dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:ring-blue-500"
            placeholder="Name"
            value={name}
          ...
          <input
            className="rounded-md border border-gray-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400
                       focus:outline-none focus:ring-2 focus:ring-blue-600
                       dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:ring-blue-500"
            placeholder="Phone"
            value={phone}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <button
          ...
          className="rounded-md bg-blue-600 px-5 py-2 text-sm font-medium text-white
                   hover:bg-blue-700 disabled:opacity-60 dark:bg-blue-500 dark:hover:bg-blue-600"
        >
      ...
      {successVisible && result && (
        <div className="mt-6 rounded-md border border-green-200 bg-green-50 p-4 dark:border-green-500/30 dark:bg-green-500/10">
